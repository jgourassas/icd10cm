defmodule Icd10cmWeb.Icd10cm_neoplasmControllerTest do
  ...
  describe "create icd10cm_neoplasm" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, Routes.icd10cm_neoplasm_path(conn, :create), icd10cm_neoplasm: @create_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.icd10cm_neoplasm_path(conn, :create), icd10cm_neoplasm: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Icd10cm neoplasm"
    end
  end
  ...
  describe "edit icd10cm_neoplasm" do
    ...
    test "renders form for editing chosen icd10cm_neoplasm", %{conn: conn, icd10cm_neoplasm: icd10cm_neoplasm} do
  ...
  describe "update icd10cm_neoplasm" do
    ...
    test "redirects when data is valid", %{conn: conn, icd10cm_neoplasm: icd10cm_neoplasm} do
      conn = put(conn, Routes.icd10cm_neoplasm_path(conn, :update, icd10cm_neoplasm), icd10cm_neoplasm: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, icd10cm_neoplasm: icd10cm_neoplasm} do
      conn = put(conn, Routes.icd10cm_neoplasm_path(conn, :update, icd10cm_neoplasm), icd10cm_neoplasm: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Icd10cm neoplasm"
    end
  end
